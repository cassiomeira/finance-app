...
export default function Goals() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createGoal.mutateAsync({
      ...
      category_id: isGlobal ? undefined : categoryId || undefined,
      month: currentDate.getMonth() + 1,
      year: currentDate.getFullYear(),
      is_global: isGlobal,
    });
    setShowForm(false);
    setAmount(''); setCategoryId(''); setIsGlobal(true);
  };

  const calculateSpent = (goal: typeof goals[0]) => {
    return transactions
      .filter(t => t.type === 'expense' && (goal.is_global || t.category_id === goal.category_id))
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Controle seus limites mensais</p>
          </div>
        ...
        {showForm && (
          ...
          >
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setIsGlobal(true)} className={`flex-1 py-2 rounded-lg ${isGlobal ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>Meta Global</button>
              <button type="button" onClick={() => setIsGlobal(false)} className={`flex-1 py-2 rounded-lg ${!isGlobal ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>Por Categoria</button>
            </div>
            {!isGlobal && (
              <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="input-finance" required={!isGlobal}>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold">{goal.is_global ? 'Meta Global' : goal.category?.name}</p>
